<?php 
	...
	if(!empty($_GET['q'])){
        ...
        WHERE kd_merchant = '$kd_merchant' AND nama_kategori LIKE '%$q%' AND status_aktif_kategori = 'Y' ORDER BY tgl_input_kategori DESC LIMIT $limit, $offset"); 
                $result = array();
        ...
        $sql	= mysqli_query($db,"SELECT*FROM merchant_kategori_produk WHERE kd_merchant = '$kd_merchant' AND status_aktif_kategori = 'Y' ORDER BY tgl_input_kategori DESC LIMIT $limit, $offset"); 
        $result = array();
        	while($row = mysqli_fetch_array($sql)){
        	array_push($result,array(
        		'id_merchant_kategori_produk'	    => $row['id_merchant_kategori_produk'],
