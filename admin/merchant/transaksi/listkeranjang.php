<?php 
	...
	$id_merchant_kategori_produk     = $_GET['id_merchant_employee'];
    	...
    	if(!empty(str_replace("&#039;","'",$_GET['q']))){
            ...
            WHERE a.kd_merchant = '$id_merchant' AND a.kd_merchant_employee = '$id_merchant_kategori_produk' AND a.kd_merchant_transaksi = '' AND a.status_transaksi_detail != '3' AND b.nama_produk LIKE '%$q%'"); 
            $result = array();
            ...
            JOIN merchant_produk b ON a.kd_merchant_produk = b.id_merchant_produk
            WHERE a.kd_merchant = '$id_merchant' AND a.kd_merchant_employee = '$id_merchant_kategori_produk' AND a.kd_merchant_transaksi = '' AND a.status_transaksi_detail != '3'"); 
            $result = array();
            	while($row = mysqli_fetch_array($sql)){
            	array_push($result,array(
            		'id_merchant_transaksi_detail'	    => $row['id_merchant_transaksi_detail'],
