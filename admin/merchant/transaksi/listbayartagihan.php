<?php 
    ...
    if($level == 2){           
            ...
            WHERE status_transaksi != 3 AND status_pembayaran ='2' AND `kd_merchant` = '$id_merchant' AND `kd_merchant_employee` = '$id_merchant_employee'
    ...
    $sql	= mysqli_query($db,"SELECT * FROM `merchant_transaksi` 
            WHERE status_transaksi != 3 AND status_pembayaran ='2' AND `kd_merchant` = '$id_merchant'
            ORDER BY tgl_input_transaksi DESC LIMIT $limit, $offset"); 
            $result = array();
            	while($row = mysqli_fetch_array($sql)){
            	     if($row['status_transaksi'] == 1){
