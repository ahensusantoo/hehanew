...
AND status_rmv_voucher='N' AND start_jam<'$jam_sekarang' AND end_jam>'$jam_sekarang' AND max_pengguna>0 AND (kd_merchant = '' OR kd_merchant = '$kd_merchant')"; 
...
if(isset($data_voucher)) {
        ...
        if ($data_voucher['status_potongan'] == "1") { //Potongan Harga
            ...
            $diskon = (int)$data_voucher['potongan_voucher'];
            $maksimal_potongan =  (int)$data_voucher['max_potongan'];
    ...
    http_response_code(400);
    $respon['pesan'] = "Voucher ini tidak bisa digunakan";
    echo json_encode($respon);
    exit();
    die();
}
