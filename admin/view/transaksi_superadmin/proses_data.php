...
if(isset($_POST['batalkan_transaksi'])){
    ...
    $data = $db->query("SELECT * FROM transaksi WHERE id_transaksi='$id'")->fetch_assoc();
    $nominal_awal = $data['total_transaksi'];
    
    $db->begin_transaction();
	    //$stmt[] = $db->query("UPDATE transaksi SET status_transaksi='3', stt_updt_tf_tiket = '1' WHERE id_transaksi='$id'");
		$stmt[] = $db->query("UPDATE transaksi SET status_transaksi='3' WHERE id_transaksi='$id'");	
  
	    $stmt[] = $db->query("UPDATE tiket SET status_tiket='2' WHERE kd_transaksi='$id'");
	    
	    $stmt[] = $db->query("INSERT INTO revisi_transaksi SET id_revisi=createID('revisi_transaksi'), keterangan_revisi='$alasan', kd_admin='$_SESSION[id_admin]', kd_transaksi='$id', jenis_revisi='2', nominal_awal='$nominal_awal', nominal_akhir='0'");

	if (in_array(false, $stmt) OR in_array(0, $stmt)) {
        $db->rollback();
        $_SESSION['notifikasi']['fail'] = "Gagal membatalkan transaksi, silahkan coba kembali!!!";
        header('Location: '.base_url().'admin-super.php?page=transaksi&action=detail&id='.$_POST['batalkan_transaksi']);
        exit();
    }else{
        $db->commit();
