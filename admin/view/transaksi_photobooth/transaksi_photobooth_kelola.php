<?php
  ...
  // PAGINATION
  $perpage = 10;
  $total_transaksi = $db->query("SELECT COUNT(*) AS jml FROM photobooth_transaksi A JOIN admin B ON A.kd_admin=B.id_admin JOIN shift C ON A.kd_shift=C.id_shift JOIN jenis_pembayaran D ON A.kd_jenis_pembayaran=D.id_jenis_pembayaran WHERE status_transaksi!='3' AND DATE(A.tanggal_photobooth_transaksi)>'$mindate' $query_tgl $query_pembeli $query_kasir $query_jenis_pembayaran")->fetch_assoc()['jml'];
  ...
  $list_transaksi = $db->query("SELECT * FROM photobooth_transaksi A JOIN admin B ON A.kd_admin=B.id_admin JOIN shift C ON A.kd_shift=C.id_shift JOIN jenis_pembayaran D ON A.kd_jenis_pembayaran=D.id_jenis_pembayaran WHERE status_transaksi!='3' AND DATE(A.tanggal_photobooth_transaksi)>'$mindate' $query_tgl $query_pembeli $query_kasir $query_jenis_pembayaran ORDER BY A.id_photobooth_transaksi DESC LIMIT $awal, $perpage")->fetch_all(MYSQLI_ASSOC);
...
//   }
  ...
  $query = "SELECT * FROM photobooth_stan WHERE status_display_photobooth='Y' AND status_remove_photobooth='N'";
...
<script type="text/javascript">
  ...
  $("#frm_transaksi").on('change', '.list_barang', function(){
    $(".btn_hapus_diskon").trigger('click');
    $("#frm_transaksi select[name=jenis_pembayaran]").val("").change();
    hitungBiaya();
  })
  ...
  $("#frm_transaksi select[name=jenis_pembayaran]").change(function(){
      ...
      if(id == ""){
          $('#frm_transaksi input[name=jumlah_dibayar]').val("Rp 0");
