<?php
  ...
  if (isset($_GET['tgl_awal']) AND isset($_GET['tgl_akhir']) AND isset($_GET['pembeli'])) {
    ...
    if ($_GET['tgl_awal'] != "" AND $_GET['tgl_akhir'] != "") {
      $query_tgl = " AND tanggal_photobooth_transaksi BETWEEN '".$_GET['tgl_awal']." 00:00:00' AND '".$_GET['tgl_akhir']." 23:59:59' ";
    }else{
      $query_tgl = "";
    }
  ...
  // PAGINATION
  $perpage = 10;
  $total_transaksi = $db->query("SELECT COUNT(*) AS jml FROM photobooth_transaksi WHERE status_transaksi!='2' $query_tgl $query_pembeli")->fetch_assoc()['jml'];
  ...
  $list_transaksi = $db->query("SELECT * FROM photobooth_transaksi A JOIN admin B ON A.kd_admin=B.id_admin JOIN shift C ON A.kd_shift=C.id_shift JOIN jenis_pembayaran D ON A.kd_jenis_pembayaran=D.id_jenis_pembayaran WHERE status_transaksi!='3' $query_tgl $query_pembeli ORDER BY A.id_photobooth_transaksi DESC LIMIT $awal, $perpage")->fetch_all(MYSQLI_ASSOC);
  ...
  $query = "SELECT * FROM photobooth_stan WHERE status_display_photobooth='Y'";
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-body">
            ...
            <hr>
            <form id="frm_transaksi" action="view/transaksi_photobooth/proses_data.php" method="POST">
              <div class="row">
                <div class="col-md-6">
                  <table>
                    <tbody>
              ...
              <button type="submit" name="tambah_transaksi_photobooth_baru" class="btn btn-block btn-info tab_ada_photophooth_dipilih">CETAK TIKET</button>
              ...
              <nav aria-label="...">
                <ul class="pagination justify-content-end">
                  ...
                  <?php if ($halamanaktif > 1): ?>
                    <li class="page-item">
                      <a class="page-link" href="?page=transaksi_photobooth&action=kelola<?php if(isset($_GET['tgl_awal'])) {echo "&tgl_awal=".$_GET['tgl_awal'];} ?><?php if(isset($_GET['tgl_akhir'])) {echo "&tgl_akhir=".$_GET['tgl_akhir'];} ?><?php if(isset($_GET['pembeli'])) {echo "&pembeli=".$_GET['pembeli'];} ?>&p=<?= $halamanaktif-1 ?>">Sebelumnya</a>
                    </li>
                  <?php else: ?>
                  ...
                  <li class="page-item disabled" aria-current="page" disabled>
                    <select class="form-control btn_ubah_halaman" data-page="?page=transaksi_photobooth&action=kelola<?php if(isset($_GET['tgl_awal'])) {echo "&tgl_awal=".$_GET['tgl_awal'];} ?><?php if(isset($_GET['tgl_akhir'])) {echo "&tgl_akhir=".$_GET['tgl_akhir'];} ?><?php if(isset($_GET['pembeli'])) {echo "&pembeli=".$_GET['pembeli'];} ?>&p=" style="height: 94%">
                  ...
                  <?php if ($halamanaktif < $jmlhalaman): ?>
                    <li class="page-item">
                      <a class="page-link" href="?page=transaksi_photobooth&action=kelola<?php if(isset($_GET['tgl_awal'])) {echo "&tgl_awal=".$_GET['tgl_awal'];} ?><?php if(isset($_GET['tgl_akhir'])) {echo "&tgl_akhir=".$_GET['tgl_akhir'];} ?><?php if(isset($_GET['pembeli'])) {echo "&pembeli=".$_GET['pembeli'];} ?>&p=<?= $halamanaktif+1 ?>">Selanjutnya</a>
                    </li>
                  <?php else: ?>
                    <li class="page-item disabled">
...
<script type="text/javascript">
    ...
    $(".btn_detail_transaksi").click(function(){
        ...
        $.ajax({
            url: 'view/transaksi_photobooth/transaksi_photobooth_detail.php?id='+id,
  ...
  $(".btn_transaksi_baru").click(function(){
      if($(this).attr("data-show") == "hide"){
        $("#tab_transaksi_baru").show(300);
        $(this).attr("data-show","show");
      }else{
        $("#tab_transaksi_baru").hide(300);
  ...
  $(".btn_gunakan_diskon").click(function(){
    ...
    if(jenis == "harga"){
      ...
      $.ajax({
          url : 'view/transaksi_photobooth/proses_data.php?cek_voucher='+kode_voucher,
          success:function(result){
