<?php
    $sess_kd_merchant = $_SESSION['kd_merchant'];

    $query = "SELECT * 
                FROM view_merchant_produk mp
                WHERE mp.kd_merchant = '$sess_kd_merchant'
                    AND mp.status_remove_produk ='N'
                    AND mp.status_display_produk = 'Y'
                ORDER BY id_merchant_produk DESC";
    $data_product = $db->query($query)->fetch_all(MYSQLI_ASSOC);
?>

<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-12">
        <h1 class="m-0 text-dark">Kelola Akun</h1>
      </div>
    </div>
  </div>
</div>

<section class="content">
  <div class="container-fluid">
    <?php if (isset($_SESSION['notifikasi'])) { notifikasi($_SESSION['notifikasi']); } ?>
    <div class="row">
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <h3 class="card-title">Daftar Merchant</h3>
          </div>
          <div class="card-body">
            <table id="example" class="table table-bordered table-striped">
              <thead>
                <tr>
                  <th>No</th>
                  <th>Kode Produk</th>
                  <th>Nama Product</th>
                  <th>Merchant Kategory</th>
                  <th>Stock Product</th>
                  <th>Harga Beli</th>
                  <th>Harga Jual</th>
                  <th>Discount</th>
                  <th>Jenis Produk</th>
                  <th>Gambar Product</th>
                  <th>Status Tampil</th>
                  <th data-searchable="false" data-orderable="false">Kelola</th>
                </tr>
              </thead>
              <tbody>
                <?php $nomor=1; ?>
                <?php foreach($data_product as $key => $val) : ?>
                <?php $id_enkripsi = enkripsiDekripsi($val['id_merchant_produk'],'enkripsi'); ?>
                    <tr>
                        <td> <?= $nomor; ?> </td>
                        <td> <?= $val['kode_produk']; ?> </td>
                        <td> <?= $val['nama_produk']; ?> </td>
                        <td> <?= $val['nama_kategori']; ?> </td>
                        <td>
                            <?php 
                                if ($val['jenis_produk'] == "2") {
                                    echo $val['stok_saat_ini']; 
                                }else{
                                    echo "Produk Non Stock";
                                } 
                            ?>
                        </td>
                        <td> <?= number_format($val['harga_beli']); ?> </td>
                        <td> <?= number_format($val['harga_produk']); ?> </td>
                        <td> 
                            <?php 
                                if($val['diskon'] == ""){
                                  echo "0";
                                } else{
                                  echo  $val['diskon']." %";
                                }
                            ?> 
                        </td>
                        <td>
                            <?php if($val['jenis_produk'] == "1") {
                                echo "Non Stock";
                            }else if($val['jenis_produk'] == "2"){
                                echo "Stock";
                            }
                            
                            ?>
                        </td>
                        <td>
                            <?php if($val['gambar_produk'] != null  ) { ?>
                              <img src="<?= base_url().'/dist/img/barang/'.$val['gambar_produk'] ?>" style="width: 100px">
                            <?php } ?>
                            
                            <?php if($val['gambar_produk'] == null  ) { ?>
                                <img src="<?= base_url().'/dist/img/barang/no_picture.jpg' ?>" style="width: 100px">
                            <?php } ?>
                        </td>
                        <td>
                            <?php  
                                if($val['status_display_produk'] == "Y") {
                                    echo "Tampil";     
                                }else{
                                    echo "DiSembunyikan";
                                }
                            ?>
                        </td>
                        <td align="center"> 
                          <a href="?page=product&action=edit&id=<?php echo $id_enkripsi ?>" data-toggle="tooltip" title="Edit">
                            <button class ="btn btn-info btn-xm" style="margin: 3px"><i class="fa fa-fw fa-edit"></i></button>
                          </a>
                          <a onclick="return confirm('Yakin data ingin dihapus ?')" href="view/product/proses_data.php?product_hapus=<?= $val['nama_produk']; ?>&id=<?php echo $id_enkripsi ?>" data-toggle="tooltip" title="Hapus">
                            <button class ="btn btn-danger btn-xm" style="margin: 3px"><i class="fa fa-fw fa-trash"></i></button>
                          </a>
                        </td>
                    </tr>
                <?php $nomor++; endforeach; ?>
              </tbody>
            </table>
          </div> 
        </div>
      </div>
    </div>
  </div>
</section>

<div class="modal fade" id="myModal" role="dialog">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">

    </div>
  </div>
</div>

<script type="text/javascript">
  $(document).ready(function(){
    $('.openPopup').on('click',function(){
      var dataURL = $(this).attr('data-href');
      $('.modal-content').load(dataURL,function(){
        $('#myModal').modal({show:true});
      });
    }); 
  });
</script>