            ...
            <table id="example" class="table table-bordered table-striped">
              ...
              <tbody>
                ...
                $sql = mysqli_query($db,"SELECT 
                  ...
                  (SELECT SUM(E.jumlah_tiket) FROM photobooth_tiket E JOIN photobooth_transaksi F ON E.kd_photobooth_transaksi=F.id_photobooth_transaksi WHERE  F.status_transaksi!='2' AND E.kd_photobooth_stan=A.kd_photobooth_stan AND DATE(F.tanggal_photobooth_transaksi) BETWEEN '$tgl_awal' AND '$tgl_akhir') AS jumlah_tiket,

                  (SELECT SUM(H.diskon) FROM photobooth_tiket G JOIN photobooth_transaksi H ON G.kd_photobooth_transaksi=H.id_photobooth_transaksi WHERE  H.status_transaksi!='2' AND G.kd_photobooth_stan=A.kd_photobooth_stan AND DATE(H.tanggal_photobooth_transaksi) BETWEEN '$tgl_awal' AND '$tgl_akhir') AS total_diskon,

                  (SELECT SUM(I.harga_satuan) FROM photobooth_tiket I JOIN photobooth_transaksi J ON I.kd_photobooth_transaksi=J.id_photobooth_transaksi WHERE  J.status_transaksi!='2' AND I.kd_photobooth_stan=A.kd_photobooth_stan AND DATE(J.tanggal_photobooth_transaksi) BETWEEN '$tgl_awal' AND '$tgl_akhir') AS total_transaksi,

                  (SELECT harga_photobooth_stan FROM photobooth_stan
                  WHERE id_photobooth_stan=A.kd_photobooth_stan) AS subtotal_transaksi

                  FROM photobooth_tiket A JOIN photobooth_transaksi B ON A.kd_photobooth_transaksi=B.id_photobooth_transaksi JOIN photobooth_stan C ON A.kd_photobooth_stan=C.id_photobooth_stan WHERE B.status_transaksi='1' AND DATE(B.tanggal_photobooth_transaksi) BETWEEN '$tgl_awal' AND '$tgl_akhir' GROUP BY A.kd_photobooth_stan
                  ");
                  ...
                  <tr>
                    ...
                    foreach($array_data_shift as $value){
                      $ambil_pershift = mysqli_query($db,"SELECT 
                        ...
                        (SELECT harga_photobooth_stan FROM photobooth_stan
                        WHERE id_photobooth_stan='$kd_photobooth_stan_temp') AS subtotal_transaksi");
                ...
                <tr>
                  ...
                  foreach($array_data_shift as $value){
                    $ambil_pershift_total = mysqli_query($db,"SELECT 
                      ...
                      AND B.kd_shift='$value->id'
                      AND DATE(B.tanggal_photobooth_transaksi) BETWEEN '$tgl_awal' AND '$tgl_akhir') AS jumlah_tiket,
                      (SELECT SUM(nominal_sebelum_diskon) FROM photobooth_transaksi
                      WHERE  status_transaksi!='2' 
                      AND kd_shift='$value->id'
                      AND DATE(tanggal_photobooth_transaksi) BETWEEN '$tgl_awal' AND '$tgl_akhir') AS total_transaksi");
