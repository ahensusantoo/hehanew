<?php
  ...
  // CEK FILTER
    ...
    $list_transaksi_string = "
        ...
        LEFT JOIN agen E ON A.id_agen = E.id_agen
        LEFT JOIN indo_cities F ON E.kota_agen = F.city_id
        WHERE status_transaksi!='3' AND DATE(A.tanggal_transaksi) > '$mindate'
            AND A.id_agen != '' 
            $query_tgl $query_pembeli $query_kasir $query_agen $query_jenis_pembayaran";

    // PAGINATION
    $perpage = 10;
    $total_transaksi = $db->query("SELECT COUNT(*) AS jml $list_transaksi_string")->fetch_assoc()['jml'];
    ...
    $list_kasir = $db->query("SELECT * FROM admin WHERE jabatan_admin='2' AND status_rmv_admin='N' ORDER BY nama_admin ASC ");
    $agen_tour = $db->query("SELECT * FROM agen WHERE status_hapus_agen='N' ORDER BY nama_agen ASC ")->fetch_all(MYSQLI_ASSOC);
                            ...
                            <form action="?page=lap_tour_guide&action=kelola" method="GET" enctype="multipart/form-data">
                                <div class="row mb-3">
                                    <div class="col-md-6 pb-3">
                                        <input type="text" name="" class="form-control form-control" id="daterange-btn" readonly>
                                    ...
                                    <div class="col-md-6 pb-3">
                                        <select class="form-control form-control select2bs4" name="agen">
                                            ...
                                            <?php foreach ($agen_tour as $key => $value): ?>
                                                ...
                                                ?>
                                                <option value="<?= $id_agen ?>">( <?= strtoupper($value['no_identitas_agen']) ?> ) - <?= strtoupper($value['nama_agen']) ?></option>
                                            <?php endforeach ?>
                                        </select>
                                    </div>
                                    <div class="col-md-6 pb-3">
                                        <select class="form-control form-control" name="jenis_pembayaran">
                                    ...
                                    <div class="col-md-6 pb-3">
                                        <select class="form-control form-control select2bs4" name="kasir">
                                ...
                                <table>
                                    <tbody>
                                        <tr>
                                            ...
                                            <td> : </td>
                                            <td>
                                                <?= tanggal_indo($_GET['tgl_awal']) ?> - <?= tanggal_indo($_GET['tgl_akhir']) ?>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>Jenis Pembayaran</td>
                                            <td> : </td>
                                            <td><?= $pembayaran_filter ?></td>
                                        </tr>
                                        <tr>
                                            <td>Kasir</td>
                                            <td> : </td>
                                        ...
                                        <tr>
                                            <td>Pembeli</td>
                                            <td> : </td>
                                            <td><?= (@$_GET['pembeli'] == "")? 'Semua Pembeli' : $_GET['pembeli'];  ?></td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($list_transaksi as $key => $value): ?>
                                        <tr>
                                            ...
                                            <td><?= $value['no_identitas_agen'] ?></td>
                                            <td><?= $value['nama_agen'] ?></td>
                                            <td><?= $value['city_name'] == null ? '-' : $value['city_name'] ?></td>
