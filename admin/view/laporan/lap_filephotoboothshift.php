...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0 text-dark">Laporan File Shift Photobooth</h1>
      </div>
    </div>
  </div>
</div>
          ...
          <div class="card-body">
            <table id="example" class="table table-bordered table-striped">
              ...
              <tbody>
                ...
                (SELECT SUM(C.jumlah_tiket) FROM photoboothambil_transaksi C WHERE C.status_transaksi='1' AND DATE(C.tanggal_photoboothambil_transaksi) BETWEEN '$tgl_awal' AND '$tgl_akhir' AND C.kd_shift=A.kd_shift ) AS jumlah_tiket ,
                
                (SELECT SUM(C.nominal_sebelum_diskon) FROM photoboothambil_transaksi C WHERE C.status_transaksi='1' AND DATE(C.tanggal_photoboothambil_transaksi) BETWEEN '$tgl_awal' AND '$tgl_akhir' AND C.kd_shift=A.kd_shift) AS total_harga,
                
                (SELECT SUM(C.diskon) FROM photoboothambil_transaksi C WHERE C.status_transaksi='1' AND DATE(C.tanggal_photoboothambil_transaksi) BETWEEN '$tgl_awal' AND '$tgl_akhir' AND C.kd_shift=A.kd_shift) AS diskon,
                
                (SELECT SUM(C.total_transaksi) FROM photoboothambil_transaksi C WHERE DATE(C.tanggal_photoboothambil_transaksi) BETWEEN '$tgl_awal' AND '$tgl_akhir' AND C.status_transaksi='1' AND C.kd_shift=A.kd_shift) AS total_transaksi
                
                FROM photoboothambil_transaksi A JOIN shift B ON A.kd_shift=B.id_shift WHERE A.status_transaksi='1' AND DATE(A.tanggal_photoboothambil_transaksi) BETWEEN '$tgl_awal' AND '$tgl_akhir' GROUP BY A.kd_shift");
...
<script type="text/javascript">
  $(function () {
    ...
    function (start, end) {
      ...
      var form = $('<form action="?page=lap-filephotoboothshift" method="post">' +
  ...
  function exportexcel(awal, akhir){
    var form = $('<form target="_blank" action="view/laporan/lap_filephotoboothshift_export_excel.php" method="post">' +
