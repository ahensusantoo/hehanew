...
<input type="hidden" name="namafile" id="namafile" value="<?= 'Laporan Gabungan File Spot Foto '.tanggal_indo($tgl_awal).' - '.tanggal_indo($tgl_akhir); ?>">
...
<table class="table2excel" data-tableName="Test Table 1">
	...
	<tbody>
		...
		while($query = mysqli_fetch_array($sql)) {
			$kd_photobooth_stan_temp = $query['kd_photoboothambil_stan'];
			...
			<tr>
				...
				foreach($array_data_shift as $value){
					$ambil_pershift = mysqli_query($db,"SELECT 
						...
						AND A.kd_photoboothambil_stan='$kd_photobooth_stan_temp'
						AND B.kd_shift='$value->id'
						AND DATE(B.tanggal_photoboothambil_transaksi) BETWEEN '$tgl_awal' AND '$tgl_akhir') AS jumlah_tiket,

						(SELECT harga_photoboothambil_stan FROM photoboothambil_stan
						WHERE id_photoboothambil_stan='$kd_photobooth_stan_temp') AS subtotal_transaksi");
