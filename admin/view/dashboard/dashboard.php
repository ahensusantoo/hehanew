...
// PHOTOBOOTH BULANAN SEMUA
$photobooth_bulanan = @$db->query("SELECT CASE WHEN SUM(total_transaksi) IS NULL THEN 0 ELSE SUM(total_transaksi) END AS jml_rupiah, CASE WHEN SUM(jumlah_tiket) IS NULL THEN 0 ELSE SUM(jumlah_tiket) END AS jml_tiket FROM photobooth_transaksi WHERE status_transaksi='1' AND MONTH(tanggal_photobooth_transaksi)='$bulan_ini'")->fetch_assoc();


// PHOTOBOOTH HARIAN SEMUA
$photobooth_harian = @$db->query("SELECT CASE WHEN SUM(total_transaksi) IS NULL THEN 0 ELSE SUM(total_transaksi) END AS jml_rupiah, CASE WHEN SUM(jumlah_tiket) IS NULL THEN 0 ELSE SUM(jumlah_tiket) END AS jml_tiket FROM photobooth_transaksi WHERE status_transaksi='1' AND DATE(tanggal_photobooth_transaksi)='$tanggal_ini'")->fetch_assoc();
    ...
    <div class="row">
      ...
      <div class="col-12 col-sm-6 col-md-6">
        <div class="info-box">
          <span class="info-box-icon bg-info elevation-1"><i class="fas fa-money-bill-wave-alt"></i></span>
          <div class="info-box-content">
            <span class="info-box-text">Omset Photobooth <br>Bulan Ini</span>
            <span class="info-box-number">
                <table>
                    <tbody>
                        <tr>
                            <td>Rp</td>
                            <td>:</td>
                            <td><?= number_format($photobooth_bulanan['jml_rupiah']) ?></td>
                        </tr>
                        <tr>
                            <td>Tiket</td>
                            <td>:</td>
                            <td><?= number_format($photobooth_bulanan['jml_tiket']) ?></td>
                        </tr>
                    </tbody>
                </table>
          </div>
      ...
      <div class="col-12 col-sm-6 col-md-6">
        <div class="info-box mb-3">
          <span class="info-box-icon bg-success elevation-1"><i class="fas fa-money-bill-wave-alt"></i></span>
          <div class="info-box-content">
            <span class="info-box-text">Omset Photobooth <br>Hari Ini</span>
            <span class="info-box-number">
                <table>
                    <tbody>
                        <tr>
                            <td>Rp</td>
                            <td>:</td>
                            <td><?= number_format($photobooth_harian['jml_rupiah']) ?></td>
                        </tr>
                        <tr>
                            <td>Tiket</td>
                            <td>:</td>
                            <td><?= number_format($photobooth_harian['jml_tiket']) ?></td>
                        </tr>
                    </tbody>
                </table>
            </span>
