...
<?php if($data_transaksi['status_transaksi'] == "2") : ?>
    <?php $data_revisi = $db->query("SELECT * FROM photobooth_revisi_transaksi A JOIN admin B ON A.kd_admin=B.id_admin WHERE kd_transaksi='$id'")->fetch_assoc(); ?>
    <center><span class="text-red">Transaksi Dibatalkan Oleh <?= @$data_revisi['nama_admin'] ?>, karena <?= @$data_revisi['keterangan_revisi'] ?></span></center>
    <hr>
<?php endif; ?>
...
<form id="frm_pembatalan" action="view/transaksi_superadmin_photobooth/proses_data.php" class="mb-5 pb-3" method="POST" style="display:none;">
    <hr>
    <div class="form-group">
        <label>Alasan Pembatalan</label>
        <input type="hidden" name="batalkan_transaksi" value="<?= enkripsiDekripsi($data_transaksi['id_photobooth_transaksi'],'enkripsi') ?>">
