...
if(isset($_POST['revisi_transaksi_satuan'])){
    ...
    if($sql){
        ...
        header('Location: '.base_url().$_SESSION['base_php'].'?page=transaksi&action=detail&id='.enkripsiDekripsi($id_transaksi,'enkripsi'));
        exit();
    }else{
...
if(isset($_POST['batalkan_transaksi'])){
    ...
    $data = $db->query("SELECT * FROM photoboothambil_transaksi WHERE id_photoboothambil_transaksi='$id'")->fetch_assoc();
    $nominal_awal = $data['total_transaksi'];
    

    $db->query("UPDATE photoboothambil_transaksi SET status_transaksi='2' WHERE id_photoboothambil_transaksi='$id'");

    $db->query("UPDATE photoboothambil_tiket SET status_tiket='2' WHERE kd_photoboothambil_transaksi='$id'");
    
    $db->query("INSERT INTO photoboothambil_revisi_transaksi SET id_revisi_ambil=createID('photoboothambil_revisi_transaksi'), keterangan_revisi_ambil='$alasan', kd_admin='$_SESSION[id_admin]', kd_transaksi_ambil='$id', jenis_revisi='2', nominal_awal='$nominal_awal', nominal_akhir='0'");


    $_SESSION['notifikasi']['success'] = "Transaksi Telah Dibatalkan";
    header('Location: '.base_url().$_SESSION['base_php'].'?page=file_transaksi_photobooth&action=kelola');
    exit();
