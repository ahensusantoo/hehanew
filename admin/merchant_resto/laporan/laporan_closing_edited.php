...
if($level == 2){
   ...
   $diskon = $db->query("SELECT SUM((A.harga_produk-A.harga_setelah_diskon)*A.jumlah_produk) AS jml FROM merchant_transaksi_detail A JOIN merchant_transaksi B ON A.kd_merchant_transaksi=B.id_merchant_transaksi WHERE 
    ...
    AND DATE(A.tgl_input_detail) BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND A.status_transaksi_detail='2'")->fetch_assoc()['jml'];
  	...
  	$gettax = $db->query("SELECT SUM(a.biaya_service) as servis, SUM(a.biaya_pajak) as pajak FROM merchant_transaksi a JOIN merchant_transaksi_detail b ON a.id_merchant_transaksi = b.kd_merchant_transaksi 
    WHERE a.kd_merchant = '$id_merchant' AND a.kd_merchant_employee = '$id_merchant_employee' 
    AND DATE(a.tgl_input_transaksi) BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND b.status_transaksi_detail = '2'")->fetch_object();
   ...
   while($row = mysqli_fetch_array($sql)){
       ...
       $servis                   = $servis + $gettax->servis;
       $pajak                    = $pajak + $gettax->pajak;
