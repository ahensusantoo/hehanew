...
if($level == 2){
   ...
   $cektax = "SELECT a.nominal_sebelum_diskon, a.diskon, SUM(a.biaya_service) as servis, SUM(a.biaya_pajak) as pajak, SUM(a.tagihan_nota) as tagihan_nota,a.jumlah_bayar,a.jumlah_item 
                  FROM merchant_transaksi a JOIN merchant_transaksi_detail b ON a.id_merchant_transaksi = b.kd_merchant_transaksi
                  WHERE a.kd_merchant ='$id_merchant' AND a.kd_merchant_employee = '$id_merchant_employee' 
                  AND DATE(a.tgl_input_transaksi) BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND b.status_transaksi_detail = '2' GROUP BY a.kd_merchant";
	$gettax = $db->query($cektax)->fetch_object();
