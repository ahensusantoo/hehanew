...
require_once('../../templates/koneksi.php');
	...
	$tagihan_nota     	   = round($_POST['tagihan_nota']);
	$jumlahbayar           = round($_POST['jumlahbayar']);
	...
	$tagihan_nota_sebelum  = round($_POST['tagihan_nota_sebelum']);
    $nomeja                = $_POST['nomeja'];
    $servis                = $_POST['servis'] ?? '';
    $pajak                 = $_POST['pajak'] ?? '';

	$nilai_servis = $tagihan_nota * (5/100);
	$nilai_servis2 = $nilai_servis + $tagihan_nota;
	$nilai_pajak = $nilai_servis2 * (10/100);
	
	if (isset($_POST['diskon'])){
	    $diskon = round($_POST['diskon']);
	} else {
	    $diskon = '0';
	}
	...
	if(isset($result[0])) {
        ...
        '$id_jenis_pembayaran', '$catatan_pesanan', '$idmerchant', '$status_pembayaran', '$idku', '$kd_shift', '$tagihan_nota_sebelum', '$diskon', '$id_referensi', '$nomeja', '$nilai_servis', '$nilai_pajak')");
